...
/obj/screen/healths/blood/Click(location, control, params)
	...
	if(ishuman(usr))
		...
		if(modifiers["left"])
			...
			if(headpercent)
				to_chat(H, "<span class='purple'>MY WOUNDS! THEY'RE MORTAL!!!</span>")
			if(burnspercent)
				to_chat(H, "<span class='orange'>IT BURNS!!! AAAGH!!</span>")
			if(bloodpercent < 100)
				to_chat(H, "<span class='red'>I'm feeling woozy...</span>")
			if(toxpercent)
				to_chat(H, "<span class='green'>Ergh... that tasted odd.</span>")
			if(oxpercent)
				to_chat(H, "<span class='grey'>CAN'T... BREATHE...</span>")
			if(H.nutrition < 0)
				to_chat(H, "<span class='red'>I need to find some food...</span>")
		if(modifiers["right"])
			if(H.mind)
