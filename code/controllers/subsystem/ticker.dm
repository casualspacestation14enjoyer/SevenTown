...
/datum/controller/subsystem/ticker/proc/checkreqroles()
	...
	var/list/required_jobs = list("Heartfelt Captain", "Corvani Captain")
	if(GLOB.debug_shit)
		required_jobs = list()
		readied_jobs = list("Heartfelt Captain", "Corvani Captain")
#ifdef DEPLOY_TEST
	required_jobs = list()
	readied_jobs = list("Heartfelt Captain", "Corvani Captain")
#endif
				...
				if(player.ready == PLAYER_READY_TO_PLAY)
					if(player.client.prefs.lastclass == V)
						if(player.IsJobUnavailable(V) != JOB_AVAILABLE)
							...
							continue
				readied_jobs.Add(V)
	if(!("Heartfelt Captain" in readied_jobs) || !("Corvani Captain" in readied_jobs))
		var/list/stuffy = list("Both Captains need to exist for a war to start.")
